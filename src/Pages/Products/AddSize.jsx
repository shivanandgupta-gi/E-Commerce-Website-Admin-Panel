...
const AddSize = () => {
    ...
    const handleSubmit=(e)=>{
        ...
        if(editId === ''){
            //data post 
            postData("/api/product/productSize/create",{name:name}).then((res)=>{
                if (res.error === false) {
        ...
        if(editId !== ''){ //means edit the data
            editData(`/api/product/updateSize/${editId}`,{name:name}).then((res)=>{
                if (res.error === false) {
                    context.openAlertBox("success",res.message)
                    setTimeout(() => { //loader run for 300ms
                        setIsLoading(false);
    ...
    const getRAMS=()=>{
         getData("/api/product/getAllProductSize").then((res)=>{
    ...
    //delete the rams
    const deleterams=(id)=>{
        deleteData(`/api/product/productSizeDelete/${id}`).then((res)=>{
            getRAMS();
            context.openAlertBox("success","Size deleted successfully");
        })
    }
    //edit the ram
    const editItem=(id)=>{
       //first find the id of rams at that id
       getData(`/api/product/productSizeById/${id}`).then((res)=>{
            ...
            <div className="card px-5 pb-5 shadow-md sm:rounded-lg bg-white w-[65%] mt-5 mb-5">
                <form className='form py-3 p-6' onSubmit={handleSubmit}>
                    <div className="col mb-4">
                        <h3 className=" text-[14px] font-[600] mb-2 px-2 text-black">PRODUCT Size</h3>
                        <input
                            type="text"
                            name='name'
                            value={name}
