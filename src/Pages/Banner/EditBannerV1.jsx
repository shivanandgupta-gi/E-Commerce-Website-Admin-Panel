...
export default function EditBannerV1() {
  ...
  useEffect(() => {
    ...
    getData(`/api/banner/${id}`).then((res) => {
      ...
      formFields.title=res.banner.title,
      formFields.Price=res.banner.Price,
      setPreviews(res.banner.images)
      formFields.images=res.banner.image,
      setproductCategory(res.banner.catId)
      formFields.catId=res.banner.catId
      setproductSubCategory(res.banner.subCatId)
      formFields.subCatId=res.banner.subCatId
      setproductThirdLevelSubCategory(res.banner.thirdsubCatId)
  ...
  const handleSubmit = (e) => {
    ...
    editData(`/api/banner/${context.isOpenFullScreenPanel.id}`, formFields).then((res) => {
      if (res?.error === false) {
        setTimeout(() => {
          setIsLoading(false);
